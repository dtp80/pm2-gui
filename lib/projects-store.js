...
function addProject (folderPath) {
  ...
  // Refuse to manage this app under PM2 via its own dashboard.
  // Starting a second instance binds the same port and crash-loops (EADDRINUSE).
  ...
  var existing = _.find(store.projects, function (p) {
    return path.resolve(p.path) === absolutePath
  })
  if (existing) {
    return existing
  }
...
function removeProjectByPath (folderPath) {
  ...
  var store = loadStore()
  var target = path.resolve(folderPath)
  var before = store.projects.length
  store.projects = store.projects.filter(function (p) {
    return path.resolve(p.path) !== target
  })
  if (store.projects.length === before) {
    return false
  }
...
function removeProjectsByPaths (paths) {
  ...
  var before = store.projects.length
  store.projects = store.projects.filter(function (p) {
    return targets.indexOf(path.resolve(p.path)) < 0
  })
  var removed = before - store.projects.length
  if (removed > 0) {
    saveStore(store)
  }
  return removed
