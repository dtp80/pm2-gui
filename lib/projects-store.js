...
function addProject (folderPath, extras) {
  ...
  if (existing) {
    if (extras.servicePort) {
      db.db.prepare('UPDATE projects SET service_port = ? WHERE id = ?')
        .run(parseInt(extras.servicePort, 10) || null, existing.id)
    }
    if (extras.serviceUrl) {
      db.db.prepare('UPDATE projects SET service_url = ? WHERE id = ?')
        .run(extras.serviceUrl, existing.id)
    }
