  ...
  function actions (action, id) {
    ...
    if (action === 'delete') {
      return pm.list({ pm2Home: self.options.pm2Home }, function (err, procs) {
        ...
        var pathsToRemove = []
        if (id === 'all') {
          pathsToRemove = (procs || []).map(function (proc) {
        ...
        pm.action({
          ...
          finishAction(actionErr, forceRefresh)
        })
      })
    }
...
Monitor.prototype._connectProcessSocket = function (socket) {
  ...
  function sendProcs () {
    if (self._cache.processes) {
      socket.emit(conf.SOCKET_EVENTS.DATA_PROCESSES, self._cache.processes, conf.NSP.PROCESS)
      return
    }
    self._refreshProcs()
  }
